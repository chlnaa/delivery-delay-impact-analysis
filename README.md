...
> SQL·Python으로 가설을 검증하고 Metabase로 KPI를 모니터링하는 엔드투엔드 파이프라인을 구축.

**데이터**: Olist Brazilian E-Commerce (Kaggle, 2016–2018) 약 10만 주문  
**이벤트 로그**: 실제 재구매 데이터가 없으므로 이벤트 로그를 시뮬레이션으로 생성하여 보완했다.  
 재구매 행동 시뮬레이션 320,759건 생성 (order_placed / order_delivered / review_submitted / repurchase)
